...
class SpampedeData {
	/**
	 * The Score of the current Game! number of spam cells eaten
	 */
	private int score;
	...
	public void eatSpam(BoardCell spamCell) {
		this.getSnakeHead().becomeBody(); // head becomes body
		this.snakeCells.addLast(spamCell);
		this.getSnakeHead().becomeHead(); // spam cell becomes Head, nothing removed from tail
		this.score ++; // add one to the score!
	}
	
	public void keepGoing(BoardCell nextCell) {
		this.getSnakeTail().becomeOpen(); // last one becomes open
		this.snakeCells.removeFirst();
		this.getSnakeHead().becomeBody();
		this.snakeCells.addLast(nextCell);
		this.getSnakeHead().becomeHead(); // next one becomes Head
	}
	...
	public BoardCell getNorthNeighbor(BoardCell cell) {
		return this.boardCells2D[cell.getRow()-1][ cell.getColumn()];
	}
	...
	public BoardCell getEastNeighbor(BoardCell cell) {
		return this.boardCells2D[cell.getRow()][ cell.getColumn()+1];
	}
	...
	public BoardCell getNextCellInDir() {
		...
		else if(this.currentMode == SnakeMode.GOING_WEST) {
	...
	public void reverseSnake() {
		LinkedList<BoardCell> newSnake = new LinkedList<BoardCell>();
		// Step 1: unlabel the head
		this.getSnakeHead().becomeBody();
		
		// Step 2: reverse the body parts
		for( BoardCell cell:this.snakeCells) {
			newSnake.addFirst(cell);
		}
		
		this.snakeCells = newSnake;
		
		// relable the head
		
		this.getSnakeHead().becomeHead();
		
		// calculate the new direction after reversing!
		
		BoardCell neck = this.getSnakeNeck();
		BoardCell head = this.getSnakeHead();
		
		if(neck.getColumn()<head.getColumn()) {
			this.setDirectionEast();
		}
		else if(neck.getColumn()>head.getColumn()) {
			this.setDirectionWest();
		}
		else if (neck.getRow()<head.getRow()) {
